...
# Ensure jq is in PATH (may be installed in ~/.local/bin)
...
if [ -f "${TEMP_FILE}" ]; then
  ...
  if [[ "${AGENTVIBES_TEST_MODE:-false}" != "true" ]]; then
    ...
    if [[ "$(uname -s)" == "Darwin" ]]; then
      ...
      # Linux/WSL: Try paplay, aplay, mpg123, or ffplay
      (paplay "${TEMP_FILE}" || aplay "${TEMP_FILE}" || mpg123 "${TEMP_FILE}" || ffplay -nodisp -autoexit "${TEMP_FILE}") >/dev/null 2>&1 &
      PLAYER_PID=$!
    fi
  fi
